...
class ExtensionServer {
    ...
    launch(request) {
        ...
        return new Promise((resolve, reject) => {
            ...
            reactNativeProjectHelper_1.ReactNativeProjectHelper.getReactNativePackageVersionFromNodeModules(mobilePlatformOptions.projectRoot)
                .then(version => {
                mobilePlatformOptions.reactNativeVersion = version;
                extProps = telemetryHelper_1.TelemetryHelper.addReactNativeVersionToEventProperties(version, extProps);
                ...
                .catch(error => {
                telemetryHelper_1.TelemetryHelper.sendErrorEvent("ReactNativePackageIsNotInstalled", errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.ReactNativePackageIsNotInstalled));
                this.logger.error(error);
                reject(error);
            });
        });
...
function requestSetup(args) {
    ...
    commandExecutor_1.CommandExecutor.ReactNativeCommand = settingsHelper_1.SettingsHelper.getReactNativeGlobalCommandName(workspaceFolder.uri);
